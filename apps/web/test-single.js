// Single post test for V2 endpoint
async function testFactCheck(postUrl) {
  ...
  try {
    ...
    result.analysis.claimAnalysis.forEach((ca, i) => {
      ...
      console.log(`     Evidence: ${ca.evidence.substring(0, 200)}${ca.evidence.length > 200 ? '...' : ''}`);
    });
...
if (!postUrl) {
  console.log('Usage: node test-single.js <X_POST_URL>');
  console.log('Example: node test-single.js https://x.com/username/status/123456789');
  process.exit(1);
}

testFactCheck(postUrl);
